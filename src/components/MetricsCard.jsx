...
export const MetricsCardProvider = ({ children }) => {
...
export const MetricsCard = ({
  ...
  const isGood = title.toLowerCase().includes("debt")
    ? !isPositive
    : isPositive;

  // Convert title to camelCase for accessing data
  const metricKey = title
    .toLowerCase()
  ...
  return (
    <>
      <div
        className={`bg-white rounded-lg shadow p-4 transition-all duration-300 cursor-pointer relative 
          ${
            isHovered
              ? "border border-blue-200 shadow-md z-10 transform -translate-y-1"
              : otherCardHovered
              ? "border border-transparent scale-[0.98] opacity-90"
              : "border border-transparent"
          }`}
        onClick={() => setShowExpanded(true)}
        onMouseEnter={() => setHoveredId(id)}
        onMouseLeave={() => setHoveredId(null)}
        style={{ minHeight: "150px" }} // 👈 Fix added: prevent shuttering
      >
        ...
        <div className="mt-2">
          ...
          {isHovered && (
            <div className="mt-2 text-center text-xs text-gray-500 flex items-center justify-center animate-fadeIn">
